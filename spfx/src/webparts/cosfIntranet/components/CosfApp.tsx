...
export default function CosfApp(props: ICosfAppProps) {
  const [activeTab, setActiveTab] = React.useState('home');

  const tabContent: Record<string, React.ReactElement> = {
    'home':             React.createElement(HomeTab,             { userName: props.userName }),
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2F5F8', fontFamily: "'Poppins', sans-serif", margin: '0 -16px' }}>
      <CosfHeader
        activeTab={activeTab}
      ...
      <footer style={{ backgroundColor: B.metallic, color: '#fff', marginTop: 64 }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '40px 32px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 32, marginBottom: 32 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                {props.logoUrl ? (
                  <img src={props.logoUrl} alt="Logo" style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover' }} />
                ) : (
                ...
                <span style={{ ...HEADLINE, fontWeight: 700, fontSize: 13 }}>{props.siteTitle}</span>
              </div>
            ...
            {[
              ...
              <div key={col.title}>
                <h4 style={{ ...BODY, fontSize: 11, fontWeight: 600, textTransform: 'uppercase' as 'uppercase', letterSpacing: '0.1em', color: B.lime, margin: '0 0 12px' }}>{col.title}</h4>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,.1)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ ...BODY, fontSize: 11, color: 'rgba(255,255,255,.4)', margin: 0 }}>© {new Date().getFullYear()} {props.siteTitle} · Employee Intranet · For internal use only</p>
